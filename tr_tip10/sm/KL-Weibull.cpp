...
// globals
const char *progname = "KL-Weibull";
...
int main (int argc, char *argv[]) {
	...
	// list of Weibull parameter vectors
 	gsl_vector **wbllist = (gsl_vector **)malloc(num * sizeof(gsl_vector *));
	...
	while (fgets(line, sizeof(line), listfile) != NULL) {
	    ...
	    line[strlen(line)-1] = '\0';
  		if (verbose)
            fprintf(stderr, "%s: reading %d Weibull parameters for %s\n", progname, dim, line);

		sprintf(modelname, "%s%s.wbl", base_path, line);
    ...
    for (unsigned int i = 0; i < cnt; i++) {
		gsl_vector_free(wbllist[i]);
    }
	free(wbllist);
    return EXIT_SUCCESS;
}

static void distance(gsl_vector **wbllist, unsigned int nimages, unsigned int dim, unsigned int start_image, unsigned int process_images) {
    for (unsigned int q = start_image; q < start_image+process_images; q++) {
		if (verbose)
			fprintf(stderr, "%s: processing query %d\n", progname, q);
		int queryisnull = gsl_vector_isnull(wbllist[q]);
		...
		for (unsigned int i = 0; i < nimages; i++) {
			...
			int candidateisnull = gsl_vector_isnull(wbllist[i]);
			if (candidateisnull) { // null vector
				store_dist(q, i, DBL_MAX);
				continue;
			}
	    	double dist = probdist_fast(wbllist, q, i, dim/2);
			store_dist(q, i, dist);
		}
	}
}
...
static double kldist(double a1, double b1, double a2, double b2) {
	double res =  -1.0 + GSL_SF_PSI_1 + gsl_sf_log(pow(a1,-b1)*b1) - gsl_sf_log(pow(a2,-b2)*b2)  + gsl_sf_gamma(1+b2/b1)*pow(a1/a2,b2) + gsl_sf_log(a1)*b1 - gsl_sf_log(a1)*b2 - 				GSL_SF_PSI_1*b2/b1;

    return (res > 0.0) ? res : 0.0;
}

static double probdist(gsl_vector **wbllist, int q, int k, unsigned int nb) {
    double dist = 0.0; 
	double *fv1 = wbllist[q]->data;
	double *fv2 = wbllist[k]->data;
...
static double probdist_fast(gsl_vector **wbllist, int q, int k, unsigned int nb) {
    double dist = 0.0; 
    double t;
	double *fv1 = wbllist[q]->data;
	double *fv2 = wbllist[k]->data;
    ...
    for (unsigned int i = 0; i < nb; i++) {
        ...
        const double lf = log(f);
        const double lg = log(g);
        const double lk = log(k);
        const double lh = log(h); 

	    t = -1.0 + GSL_SF_PSI_1 + 
            lg - g*lf -
            lk + k*lh +
	        tgamma(1.0+k/g)*pow(f/h,k) + 
	        lf*(g - k) - 
	        GSL_SF_PSI_1*k/g;
...
static void *distance_thread(void *thread_params) {
    ...
    distance(m->wbllist, p->num_images, m->dim, p->start_image, p->process_images);
