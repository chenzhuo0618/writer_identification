...
// globals
const char *progname = "KL-Rayleigh";
...
int main (int argc, char *argv[]) {
	...
	// list of Rayleigh parameter vectors
 	gsl_vector **raylist = (gsl_vector **)malloc(num * sizeof(gsl_vector *));
	...
	while (fgets(line, sizeof(line), listfile) != NULL) {
	    ...
	    line[strlen(line)-1] = '\0';
  		if (verbose)
            fprintf(stderr, "%s: reading %d Rayleigh parameters for %s\n", progname, dim, line);

		sprintf(modelname, "%s%s.ray", base_path, line);
		...
		gsl_vector *ray = gsl_vector_calloc(dim);
		if (gsl_vector_fread(mfile, ray) == GSL_EFAILED) {
    ...
    for (unsigned int i = 0; i < cnt; i++) {
		gsl_vector_free(raylist[i]);
    }
	free(raylist);
    return EXIT_SUCCESS;
}

static void distance(gsl_vector **raylist, unsigned int nimages, unsigned int dim, unsigned int start_image, unsigned int process_images) {
    for (unsigned int q = start_image; q < start_image+process_images; q++) {
		if (verbose)
			fprintf(stderr, "%s: processing query %d\n", progname, q);
		int queryisnull = gsl_vector_isnull(raylist[q]);
		...
		for (unsigned int i = 0; i < nimages; i++) {
			...
			int candidateisnull = gsl_vector_isnull(raylist[i]);
			if (candidateisnull) { // null vector
				store_dist(q, i, DBL_MAX);
				continue;
			}
	    	double dist = probdist(raylist,q,i,dim/2);
			store_dist(q, i, dist);
		}
	}
}
...
static double probdist(gsl_vector **raylist, int q, int k, unsigned int nb) {
    double dist = 0.0; 
	double *fv1 = raylist[q]->data;
	double *fv2 = raylist[k]->data;
...
static void *distance_thread(void *thread_params) {
    ...
    distance(m->raylist, p->num_images, m->dim, p->start_image, p->process_images);
