function  out=stex_data(Jmax,maxclasses)
...
for d=3:min(2+maxclasses,size(filelist,1))
	if filelist(d).isdir ~=true
		...
		if strcmp( fname(size(fname,2)-3:size(fname,2)) ,'.pnm' ) == 1 || strcmp( fname(size(fname,2)-3:size(fname,2)) ,'.gif' ) == 1
