...
%曹海哥正常
pdata_path='G:\writing\fixed\旋转\轻度褶皱\90\mat\practiceimagebase';%cut库
tdata_path='G:\writing\fixed\旋转\轻度褶皱\90\mat\testimagebase';%cut库
%曹海哥重度
...
for classid=1:classnum
    ...
    if(classid<10)
        ...
        ClassFeatureMatrix=GainTrainImageFeature(class_name,nlevel);
    else
...
for classid=1:classnum
    if(classid<10)
        ...
        ClassFeatureMatrix=GainImageFeatureArr(class_name,classid,nlevel);
    else
...
[AverRetrievalRate2,RR2]=Gain_AverageRetrievalRate(SortingResult)
toc;
